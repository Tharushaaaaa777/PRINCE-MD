...
cmd({
    ...
    try {
        await reply("🔍 Checking for PRINCE-MD updates...");
        ...
        if (latestCommitHash === currentHash) {
            return reply("✅ Your PRINCE-MD bot is already up-to-date!");
        }

        await reply("🚀 Updating PRINCE-MD Bot...");

        // Download the latest code
        const zipPath = path.join(__dirname, "latest.zip");
        const { data: zipData } = await axios.get("https://github.com/Tharushaaaaa777/THARUSHA-XMD/archive/main.zip", { responseType: "arraybuffer" });
        ...
        const sourcePath = path.join(extractPath, "PRINCE-MD-main");
